...
describe("ExchangeAgent", function () {
  before(async function () {
    ...
    this.mockUNO = this.MockUNO.attach(UNO.kava_alpha)
    this.mockUSDT = this.MockUSDT.attach(USDT.kava_alpha)
  ...
  beforeEach(async function () {
    ...
    //   USDT.kava_alpha,
    //   WETH_ADDRESS.kava_alpha,
    //   UNO_USDT_PRICE_FEED.kava_alpha,
    //   UNISWAP_ROUTER_ADDRESS.kava_alpha,
    //   UNISWAP_FACTORY_ADDRESS.kava_alpha,
    //   this.signers[0].address,
    // )

    this.exchangeAgent = await this.ExchangeAgent.attach(exchangeAgentDeployment.address)
  })
  ...
  describe("token exchange test", function () {
    ...
    it("Should check UNO/USDT price feed", async function () {
      console.log("[exchangeAgent address]", this.exchangeAgent.address)
      const priceOracleAddr = await this.exchangeAgent.oraclePriceFeed()
      console.log("[mock uno address]", this.mockUNO.address)
      const USDT_value = await this.exchangeAgent.getNeededTokenAmount(UNO.kava_alpha, USDT.kava_alpha, getBigNumber(5000))
      const balanceUNO = await this.mockUNO.balanceOf("0x6C641CE6A7216F12d28692f9d8b2BDcdE812eD2b")
      console.log("[usdt value]", balanceUNO.toString())
    })
