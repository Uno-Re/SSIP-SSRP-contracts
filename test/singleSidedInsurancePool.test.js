...
describe("SingleSidedInsurancePool", function () {
  ...
  beforeEach(async function () {
    ...
    this.usdtOwner = await ethers.getSigner("0x8894E0a0c962CB723c1976a4421c95949bE2D4E3")
    ...
    await this.mockUSDT
      .connect(this.usdtOwner)
      .transfer(this.signers[0].address, getBigNumber("500000"), { from: this.usdtOwner.address })
    await this.mockUNO.connect(this.signers[1]).faucetToken(getBigNumber("500000000"), { from: this.signers[1].address })
    await this.mockUSDT
      .connect(this.usdtOwner)
      .transfer(this.signers[1].address, getBigNumber("500000"), { from: this.usdtOwner.address })
    await this.mockUNO.connect(this.signers[2]).faucetToken(getBigNumber("500000000"), { from: this.signers[2].address })
    await this.mockUSDT
      .connect(this.usdtOwner)
      .transfer(this.signers[2].address, getBigNumber("500000"), { from: this.usdtOwner.address })
    ...
    await (
      await this.mockUSDT
        .connect(this.usdtOwner)
        .approve(UNISWAP_ROUTER_ADDRESS.sepolia, getBigNumber("10000000"), { from: this.usdtOwner.address })
    ).wait()
    ...
    this.singleSidedInsurancePool = await ethers.getContractAt(
      ...
      "0x99EBb2865760325a078E8AEbE10D3bd066e7b79e",
    )
  ...
  describe("SingleSidedInsurancePool Actions", function () {
    ...
    describe("SingleSidedInsurancePool Staking", function () {
      it("Should enter in pool and check pending reward and totalCapital in CapitalAgent after generate 10000 blocks", async function () {
        ...
        await this.singleSidedInsurancePool.connect(this.signers[0]).enterInPool(getBigNumber("8500"))
      ...
      it("should harvest to self address after 10000 block", async function () {
        ...
        expect(poolBalance1).to.equal(getBigNumber("85000")) //erradooooooooooooo
        const poolBalance2 = await riskPool.balanceOf(this.signers[1].address)
        expect(poolBalance2).to.equal(getBigNumber("10000"))
        await advanceBlockTo(beforeBlockNumber + 10000000000000000)
        const afterBlockNumber = await ethers.provider.getBlockNumber()
    ...
    describe("SingleSidedInsurancePool withdraw", function () {
      ...
      it("Should not claim within 10 days since WR", async function () {
        await this.singleSidedInsurancePool.connect(this.multisig).setLockTime(3600 * 24 * 10)
        // signer 0 submit WR for the 1000 UNO
        await this.singleSidedInsurancePool.leaveFromPoolInPending(getBigNumber("1000"))
    ...
    describe("RiskPool LP Token Tranfer", function () {
      ...
      it("Should not allow transfer risk pool LP token when greater than the blance - WR", async function () {
        ...
        await expect(riskPool.transfer(this.signers[3].address, getBigNumber("5000"))).to.be.revertedWith(
    ...
    describe("SingleSidedInsurancePool migrate", function () {
      beforeEach(async function () {
        ...
        await this.singleSidedInsurancePool.connect(this.multisig).setMigrateTo(this.migrate.target)

        await this.mockUSDT.connect(this.usdtOwner).transfer(this.signers[8].address, getBigNumber("10000"))

        await this.mockUSDT
          .connect(this.this.signers[8].address)
          .approve(this.singleSidedInsurancePool.target, getBigNumber("10000"))

        await this.singleSidedInsurancePool.connect(this.signers[8].address).enterInPool(getBigNumber("10000"))

        await this.singleSidedInsurancePool.connect(this.signers[8].address).leaveFromPoolInPending(getBigNumber("1000"))
      })
    ...
    describe("SingleSidedInsurancePool Staking and Migrate", function () {
      beforeEach(async function () {
        ...
        //killing pool
        await this.singleSidedInsurancePool.connect(this.multisig).killPool()
      ...
      it("withdraw from v3", async function () {
        ...
        ).changeTokenBalances(this.mockUSDT, [this.signers[0].address], [userInfoV3.amount])
