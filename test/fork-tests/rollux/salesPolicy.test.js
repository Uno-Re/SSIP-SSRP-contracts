...
describe("SalesPolicy", function () {
  ...
  beforeEach(async function () {
    ...
    encodedCallData = await this.capitalAgent.interface.encodeFunctionData("addPoolWhiteList", [
    ...
    await expect(this.multisig.executeTransaction(this.txIdx)).to.be.revertedWith("cannot execute tx")

    await expect(this.multisig.confirmTransaction(this.txIdx, false)).to.be.revertedWith("tx already confirmed")
    ...
    encodedCallData = await this.capitalAgent.interface.encodeFunctionData("setSalesPolicyFactory", [
      this.salesPolicyFactory.target,
    ])
...

    {
      ...
      const index = await this.multisig.getTransactionCount()
      await (await this.multisig.submitTransaction(this.salesPolicyFactory.target, 0, encodedCallData)).wait()
      await (await this.multisig.confirmTransaction(index, false)).wait()
      await (await this.multisig.connect(this.signers[1]).confirmTransaction(index, true)).wait()
      this.txIdx++
