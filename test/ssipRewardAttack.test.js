...
describe("SingleSidedInsurancePool", function () {
  ...
  describe("SingleSidedInsurancePool Harvest Attack Actions", function () {
    beforeEach(async function () {
      ...
      await this.singleSidedInsurancePool.connect(this.signers[2]).enterInPool(this.signers[2].address, getBigNumber("1000"), {from: this.signers[2].address})
    })
    ...
    describe("SingleSidedInsurancePool harvest attack", function () {
      it("should harvest to self address after 10000 block", async function () {
        ...
        await this.singleSidedInsurancePool.connect(this.signers[1]).enterInPool(this.signers[1].address, getBigNumber("10000"), {from: this.signers[1].address})
      ...
      it("should revert harvest through malicious contract after 10000 block", async function () {
        ...
        await this.singleSidedInsurancePool
          .connect(this.signers[1])
          .enterInPool(this.signers[1].address, getBigNumber("8500"), { from: this.signers[1].address })
