...
describe("CLaimsDao SalesPolicy", async function () {
  ...
  beforeEach(async function () {
    ...
    this.optimisticOracleV3 = await ethers.getContractAt(OptimisticOracleV3Abi, OPTIMISM_ORACLE_V3.sepolia)
    this.AddressWhitelist = await ethers.getContractAt("IAddressWhitelist", ADDRESS_WHITELIST_UMA.sepolia)
    ...
    await this.singleSidedInsurancePool
      ...
      .createRewarder(this.signers[0].address, this.rewarderFactory.target, this.mockUNO.target)
    ...
    await this.singleSidedInsurancePool
      .connect(this.multisig)
      .grantRole(await this.singleSidedInsurancePool.CLAIM_PROCESSOR_ROLE(), this.payoutRequest.target)
    ...
    await this.salesPolicyFactory
      .connect(this.multisig)
      .newSalesPolicy(this.exchangeAgent.target, this.premiumPool.target, this.capitalAgent.target)
    ...
    await (await this.mockUSDT.approve(this.salesPolicyAddress, getBigNumber("100000000000000"))).wait()
    ...
    await this.singleSidedInsurancePool2
      .connect(this.multisig)
      .createRewarder(this.signers[0].address, this.rewarderFactory.target, this.mockUNO.target)
    ...
    await this.singleSidedInsurancePool1
      .connect(this.multisig)
      .grantRole(await this.singleSidedInsurancePool1.CLAIM_PROCESSOR_ROLE(), this.payoutRequest1.target)
    await this.singleSidedInsurancePool2
      .connect(this.multisig)
      .grantRole(await this.singleSidedInsurancePool2.CLAIM_PROCESSOR_ROLE(), this.payoutRequest2.target)
  ...
  describe("Sales policy Action", async function () {
    ...
    it("Should burn policy when again clain for policy to full fill coverage", async function () {
      expect(await this.salesPolicy.balanceOf(this.signers[0].address)).to.equal(2)
      await this.payoutRequest.setFailed(true)
      let unoAmount = await this.exchangeAgent.getNeededTokenAmount(
        this.mockUSDT.target,
        this.mockUNO.target,
        getBigNumber("101", 6),
      )
      ...
      let unoAmount1 = await this.exchangeAgent.getNeededTokenAmount(
        this.mockUSDT.target,
        this.mockUNO.target,
        getBigNumber("100", 6),
      )
    ...
    it("Withdrawal of position of old underwriting LPs during and after a claim payout ", async function () {
      ...
      await expect(this.singleSidedInsurancePool1.connect(this.signers[0]).leaveFromPending(getBigNumber("150", 6))).not.to.be
        .reverted
    })
    it("Withdrawal of position of new underwriting LPs after a claim payout", async function () {
      ...
      console.log(
        "balance of riskPool before enter in Pool after settlement",
        await this.mockUNO.balanceOf(this.riskpool1.target),
      )
      let amount = await this.capitalAgent.checkCapitalByMCR(this.singleSidedInsurancePool1.target, getBigNumber("200", 6))
      console.log("a", amount)

      await expect(this.singleSidedInsurancePool1.connect(this.signers[4]).leaveFromPoolInPending(getBigNumber("200", 6))).not.to
        .be.reverted
      const currentDate = new Date()
      const afterTenDays = new Date(currentDate.setDate(currentDate.getDate() + 11))
      const afterTenDaysTimeStampUTC = new Date(afterTenDays.toUTCString()).getTime() / 1000
      ...
      const signer4Amount = (await this.singleSidedInsurancePool1.getWithdrawRequestPerUser(this.signers[4].address))
        .pendingAmount
    ...
    it("pool withdraw amount calculation", async function () {
      ...
      await expect(this.singleSidedInsurancePool1.connect(this.signers[6]).leaveFromPoolInPending(getBigNumber("200", 6))).not.to
        .be.reverted
      ...
      await expect(this.singleSidedInsurancePool1.connect(this.signers[5]).leaveFromPoolInPending(getBigNumber("200", 6))).not.to
        .be.reverted
      ...
      await expect(this.singleSidedInsurancePool1.connect(this.signers[4]).leaveFromPoolInPending(withdrawableAmount + 1)).not.to
        .be.reverted
    ...
    it("Will revert when pool capital will less than SCR", async function () {
      ...
      const LPPrice = ethers.formatEther(lpprice)
      await expect(
        this.singleSidedInsurancePool1
          .connect(this.signers[1])
          .leaveFromPoolInPending(Math.trunc(Number(getBigNumber("500", 6)) * LPPrice)),
      ).not.to.be.reverted
      const signer1Amount = (await this.singleSidedInsurancePool1.getWithdrawRequestPerUser(this.signers[1].address))
        .pendingAmount
      const withdrawableAmount = Math.trunc(Number(withdrawamount) * LPPrice)
      console.log("LPPrice", LPPrice)
      console.log("withdrawableAmount", withdrawableAmount)
